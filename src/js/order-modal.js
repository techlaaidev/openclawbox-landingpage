...
(function () {
  ...
  const PROVINCES = {
    ...
    "Đắk Nông": "inter",
    "Lâm Đồng": "inter",
    "TP. Hồ Chí Minh": "inter",
    "Bình Dương": "inter",
    "Đồng Nai": "inter",
    "Bà Rịa - Vũng Tàu": "inter",
    "Long An": "inter",
    "Tây Ninh": "inter",
    "Bình Phước": "inter",
    "Tiền Giang": "inter",
  ...
  function interceptCTAButtons() {
    const orderButtons = document.querySelectorAll(
      ".pricing-tier-cta .btn, .hero-cta .btn-primary, .cta-buttons .btn-primary",
    );
  ...
  function updateBreakdownUI() {
    ...
    if (expressRow) {
      expressRow.style.display =
        expressCheckbox && expressCheckbox.checked ? "flex" : "none";
    }
  ...
  async function submitOrder() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("[Order] ✅ Order submitted:", {
          name,
          phone,
          tier,
          refCode,
  ...
  function showPayment(tier, amount, orderId, phone, refCode) {
    ...
    if (accountEl)
      accountEl.textContent = BANK_ACCOUNT.replace(/(.{4})/g, "$1 ").trim();
